...
export function RepeatedList({ isOpen, onClose, stickers }: RepeatedListProps) {
  ...
  const counts = useMemo(() => {
    ...
    Object.entries(stickers).forEach(([id, s]) => {
      ...
      agg[norm] = (agg[norm] || 0) + s;
    });
    return agg;
  }, [stickers]);

  const repeatedByTeam = useMemo(() => {
    const result: { team: string; stickers: { id: string; num: string; count: number }[] }[] = [];

    // Group by standard teams
    TEAMS.forEach((teamName) => {
      const teamRepeated: { id: string; num: string; count: number }[] = [];
    ...
    // Special: FWC
    const fwc: { id: string; num: string; count: number }[] = [];
    ...
    fwcNums.forEach((numStr) => {
      ...
      const count = counts[norm] || 0;
      if (count > 1) {
        fwc.push({ id, num: numStr, count: count - 1 });
      }
    });
    if (fwc.length > 0) result.push({ team: 'FWC', stickers: fwc });

    // Special: Coca-Cola
    const cocacola: { id: string; num: string; count: number }[] = [];
    ...
    ccNums.forEach((numStr) => {
      ...
      const count = counts[norm] || 0;
      if (count > 1) {
        cocacola.push({ id, num: numStr, count: count - 1 });
      }
    });
              ...
              {repeatedByTeam.length === 0 ? (
                ...
                <div className="space-y-4">
                  {repeatedByTeam.map((group, idx) => {
                    const groupNums = getStickerNumbers(group.team);
                    let teamOwned = 0;
                    groupNums.forEach(num => {
                      const id = group.team === 'FWC' ? `FWC-${num}` : (group.team === 'Coca-Cola' || group.team === t('album.coca_cola')) ? `CC-${num}` : `${group.team}-${num}`;
                      const norm = normalizeStickerId(id);
                      if ((counts[norm] || 0) >= 1) teamOwned++;
                    });
                    const isAllOwned = teamOwned === groupNums.length;
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className={cn(
                                  ...
                                  isAllOwned ? "bg-green-500/10 text-green-500" : "bg-blue-500/10 text-blue-500"
                                )}>
                                  {isAllOwned ? t('repeated.complete') : t('repeated.incomplete')}
                                </span>
                              </div>
                            </div>
                          </div>
